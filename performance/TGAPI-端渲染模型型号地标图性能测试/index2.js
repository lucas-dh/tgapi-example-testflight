...
// 基础经度
var baseLon = 116.2921691316575;
// 基础纬度
var baseLat = 39.82792496815947;
// 基础高度
var baseAlt = 0.01;
// 增加的经度
var deltaLon1 = 0.0000038;
// 增加的纬度
var deltaLat1 = 0.000028;
// 增加的经度2
var deltaLon2 = 0.000038;
// 增加的纬度2
var deltaLat2 = 0.0000028;
...
function init() {
  ...
  let sceneId = 'dbr500sp';
  let tokenUrl = `https://test.tuguan.net/api/user/v1/visitorScene/${sceneId}`;
      ...
      window.appInstance.initService(
        {
          ...
          url: 'http://test.tuguan.net/publish/scene/api/' + sceneId,
        },
        (result) => {
          operationUIElement();
          getTwinEasyCategory();
...
function connectTwinEasyServer() {
  let jsonData = {
    name: 'TwinEasyServer',
    //address: 'http://172.16.5.30:5000/v1/public/location/th1s0kkzk3copxe0/twinEasyCategoryId/{iconModelId}',
    address: 'http://172.16.5.30:5000/v1/public/location/wne03njzu8l74s3n/twinEasyCategoryId/{iconModelId}',
    token: 'ABCDEFG',
  };
...
function btnAddClick4() {
  ...
  countLine.textContent = '孪易图层个数： ' + twinEasyCategory.length + ' 个';
  txtLogs.appendChild(countLine);
...
/**
 * ### 添加对象 按照孪易图层数量添加同一种 drawcall 少的对象
 */
function btnAddClick5() {
  ...
  let addedCount = 0;
  for (let i = 0; i < twinEasyCategory.length; i++) {
    let layer = getLayerDefine(i, 'wl9jn7maawi9krum');
...
/**
 * ### 添加对象 按照孪易图层数量添加同一种 drawcall 多的对象
 */
function btnAddClick6() {
  // 记录开始时间
  setUnicallStartTime('添加');

  let addedCount = 0;
  for (let i = 0; i < 4; i++) {
    let layer = getLayerDefine(i, 'hzkxxdqnmhppm8f8');

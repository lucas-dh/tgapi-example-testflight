    ...
    methods: {
        ...
        window.appInstance.initService(
          {
            ...
            mode: 'streaming',
            token: streaming2024Config.airpotToken, //StreamingServer服务器获取token
      ...
      setModelTransform2Default() {
        ...
        let jsonData = {
          ...
          coordZ: 23.5, // Z 轴高度（单位：米）  // 如果值为null，或者没有传递这个属性，则表示不发生改变
          rotationX: 353, // X 轴旋转度数（单位：度）  // 0正北，值域 -360~360  // 如果值为null，或者没有传递这个属性，则表示不发生改变
          ...
          duration: 0, // 持续时长（单位：秒），表示花费多少时间长度，变化到目标值，值域范围大于0，可以是小数
        };
      ...
      setModelTransform2(useDuration = true, callback) {
        ...
        let jsonData = {
          ...
          coordType: 0, // XY 轴坐标类别，0：X 经度，Y 纬度；1：X 米，Y米，相对世界00点
          coordTypeZ: 0, // Z 轴坐标类别，0：相对 3D 世界 0 海拔；1：相对 3D 世界地表；2：相对 3D 模型表面（单位：米）
          rotationX: heading, //heading, // X 轴旋转度数（单位：度）  // 0正北，值域 -360~360  // 如果值为null，或者没有传递这个属性，则表示不发生改变
          rotationY: 0, // Y 轴旋转度数（单位：度）  // 0水平，值域 -360~360  // 如果值为null，或者没有传递这个属性，则表示不发生改变
          rotationZ: 0, // Z 轴旋转度数（单位：度）  // 0水平，值域 -360~360  // 如果值为null，或者没有传递这个属性，则表示不发生改变
          duration: 0, // 持续时长（单位：秒），表示花费多少时间长度，变化到目标值，值域范围大于0，可以是小数
        };

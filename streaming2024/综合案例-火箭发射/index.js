...
var articulations = {
  // 一级分离: {
  // start: new Date('2009-07-14 18:17:50').getTime(), // 关节开始播放时间
  // end: new Date('2009-07-14 18:17:53').getTime(), // 关节结束播放时间（开始加持续时间）
  ...
  // },
  // 整流罩脱离: {
  // start: new Date('2009-07-14 18:18:20').getTime(),
  // end: new Date('2009-07-14 18:18:23').getTime(),
  // state: 0,
  // duration: 3,
  // min: 0,
  // max: 100,
  // },
  // 二级分离: {
  // start: new Date('2009-07-14 18:25:02').getTime(),
  // end: new Date('2009-07-14 18:25:05').getTime(),
  // state: 0,
  // duration: 3,
  // min: 0,
  // max: 100,
  // },
  // 三级分级: {
  // start: new Date('2009-07-14 18:26:02').getTime(),
  // end: new Date('2009-07-14 18:26:05').getTime(),
  // state: 0,
  // duration: 3,
  // min: 0,
  // max: 100,
...
function addModelTrailLayer() {
  ...
  appInstance.uniCall('addModelTrailLayer', jsonData, (result) => {
    ...
    // 视角跟随模型
    initFollowingCameraBtn('spacecraftId');
...
function removeModelTrailLayerCoord() {
  ...
  let jsonData = {
    overlayType: 'modelTrailLayer', // 图层对象类别
    idLayer: 'ModelTrailLayer', // 图层对象id，只支持带有子对象的图层
    id: '', // 需要删除的图层子对象id
...
function followingCamera(relative) {
  ...
  let jsonData = {
    modelId: 'spacecraftId', // 镜头跟踪的模型Id，镜头和模型之前保持相对静止关系，支持运动的模型
    distance: 10, // 镜头与被跟踪物体的距离(单位:米)
...
// 飞机列表点击事件
function initFollowingCameraBtn(id) {
  let jsonData = {
    modelId: id, // 镜头跟踪的模型Id，镜头和模型之前保持相对静止关系，支持运动的模型
    distance: 10, // 镜头与被跟踪物体的距离(单位:米)
    distanceMin: -10, // 镜头与被跟踪物体的最近距离(单位:米)
    distanceMax: 50000, // 镜头与被跟踪物体的最远距离(单位:米)
    pitch: 20, //镜头俯仰角(5~89)
...
// 设置关节
function setArticulation(isReverse) {
  // 1、首先判断进度是正向还是逆向（也可通过传入参数）
  if (isReverse) {
    // 2.1、如果为逆向，则遍历所有关节（动画）：
    for (const key in articulations) {
      if (currentTime.getTime() < articulations[key].start && articulations[key].state) {
        // 2.1.1 开始时间大于当前时间并已设置（播放）过，统一瞬间设置（播放）并标记为未播放【逆向调整】
        let jsonData = {
          id: 'spacecraftId',
        ...
        articulations[key].state = 0;
      } else if (currentTime.getTime() >= articulations[key].start && currentTime.getTime() < articulations[key].end && articulations[key].state) {
        // 2.1.2 开始时间小于并且结束时间大于当前时间并已设置（播放）过，统一瞬间复位为未播放并正常设置（播放）后记录为已播放
        let jsonData = {
          id: 'spacecraftId',
          layerId: 'ModelTrailLayer',
          duration: articulations[key].duration,
          data: [
            {
              articulation: key,
              type: 'float',
    ...
    // 2.2、如果为正向，则遍历所有关节（动画）：

    for (const key in articulations) {
      if (currentTime.getTime() > articulations[key].end && !articulations[key].state) {
        // 2.2.1 结束时间小于当前时间并且未设置（播放）过的，统一瞬间设置（播放）完毕并标记状态为已播放【正向跳帧】
        let jsonData = {
          id: 'spacecraftId',
          layerId: 'ModelTrailLayer',
          duration: 0,
        ...
        articulations[key].state = 1;
      } else if (currentTime.getTime() >= articulations[key].start && !articulations[key].state) {
        // 2.2.2 开始时间小于或等于当前时间的，正常设置（播放）并标记为已播放
        let jsonData = {
          id: 'spacecraftId',
          layerId: 'ModelTrailLayer',
          duration: articulations[key].duration,
        ...
        // 2.2.3 开始时间大于当前时间的并且已经设置（播放）过的，统一瞬间复位为未播放状态【复位后期效果节点，重复播放等复位】【每次重新播放就需要先复位所有】
        let jsonData = {
          id: 'spacecraftId',
          layerId: 'ModelTrailLayer',
          duration: 0,

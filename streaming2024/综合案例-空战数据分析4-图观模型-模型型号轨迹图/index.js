  ...
  window.appInstance.initService(
    {
      ...
      mode: 'streaming',
      token: 'vaD4WxTc', //StreamingServer服务器获取token:1204：vaD4WxTc  1124：Qln096G9  1125：MKf5BecX   1216：aBndxtU7
      url: 'http://172.16.1.225:9090', //StreamingServer服务器接口地址和端口，需要进行替换为实际地址
    },
    (result) => {
...
function connctServer() {
  let jsonData = {
    name: 'luanyi',
    address: 'http://172.16.5.30:5000/v1/public/location/kmmhsw7m6i1ls51e/twinEasyCategoryId/{iconModelId}',
    token: 'ABCDEFG',
  };
...
function addEventHandler() {
  ...
  appInstance.uniCall(
    'addEventListener',
    {
      eventName: 'onIconModelTrailLayerClick', // 事件名称 见图观官网统一API开发手册
...
function addModelTrailLayer(category) {
  let jsonData = {
    ...
    legends: [
      {
        name: 'red',
        iconModelId: category === 'missile' ? '8mzntycphh2iko0e' : 'a061u9ld34jq8ddi',
        labelScale: 1,
        iconScale: 1,
      },
      {
        name: 'blue',
        iconModelId: category === 'missile' ? 'y79uai4vdpwtytfb' : 'jaufu52ken4ulw2d',
        labelScale: 1,
        iconScale: 1,
      },
    ],

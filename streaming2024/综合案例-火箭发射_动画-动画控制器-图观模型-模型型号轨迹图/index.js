...
function onTimelineClick(e, index) {
  ...
  // 获取当前点击动画的名称
  let currentClickAnimationName = timelineAnimationList[index].animationName;
  ...
  // 设置关节数据
  setArticulation(currentClickAnimationName);
 ...
 * 设置模型的关节状态。
 *
 * @param {string} currentClickAnimationName - 当前时间轴点击的动画名称。
 *
 * 该函数根据当前点击的动画名称占时间轴动画排序数组的索引值和关节所属的动画名称占排序数组的索引值比较，设置关节的状态。
 * 如果当前点击动画名称所占的索引值大于关节所属动画名称所占的索引值，则将关节状态设置为完成状态，否则将关节状态设置为初始状态。然后通过 `uniCall` 方法调用
 * `setModelArticulation` 接口来更新模型的关节状态。
 */
function setArticulation(currentClickAnimationName) {
  ...
  articulationItems.forEach((item) => {
...
function playAnimation() {
  ...
  if (currentAnimationData && currentAnimationData.useAnimation && !currentAnimationData.state) {
    ...
    if (isRunSetArticulation) {
      setArticulation(currentAnimationData.animationName);
    }
    ...
    let jsonData = {
      id: 'spacecraftId',
      layerId: 'ModelTrailLayer',
      name: currentAnimationData.animationName,
...
function getModelAnimationAndSetArticulation() {
  ...
  appInstance.uniCall('getModelAnimation', jsonData, (result) => {
    // 获取模型的全部动画数据 设置关节的数据
    // 缓存关节项数据到全局变量：articulationItems
    result.data.forEach((item) => {
      ...
      item.articulationItems.forEach((t) => {
        ...
        articulationItems.push(t);
      });
    });

    // console.log('关节项数据 data', articulationItems);
  });

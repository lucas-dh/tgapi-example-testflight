...
var airEndTime = ''; // 飞机结束的时间
var envTimer = undefined;
var airDataLength = 0; // 正常飞机的数量
var missileDataLength = 0; // 导弹的数量
var missileLegend = []; // 导弹图例数据（后续如果导弹区分红蓝方时会使用）
var destroyDataLength = 0; // 销毁飞机的数量
var airLegend = []; // 飞机图例数据（后续如果飞机区分红蓝方时会使用）
var destroyDataDataLength = 0;
  ...
  window.appInstance.initService(
    {
      ...
      mode: 'streaming',
      token: streaming2024Config.georgiaToken, //StreamingServer服务器获取token
...
function addModelTrailLayer(category) {
  ...
  let startData = this[category + 'Data'][formatDate(currentTime, 'DD-M-YYYY HH:mm:ss')];
  if (startData) {
...
function removeModelTrailLayerCoord() {
  let current = formatDate(currentTime, 'DD-M-YYYY HH:mm:ss');
  var ids = [];
  // 销毁飞机
  if (!destroyData[current]) return;
  destroyData[current].forEach((data) => {
    ids.push(data.planeID);
  });
  let jsonData = {
    overlayType: 'modelTrailLayer', // 图层对象类别
  ...
  if (!addLandmark) {
    ...
    destroyData[current].forEach((record) => {
      jsonDataLayer.data.push({
        id: record.planeID,
        type: redAirArray.includes(record.planeID) ? 'redplane' : 'blueplane',
        label: record.planeID,
        coord: [record.Lon, record.Lat],
        coordZ: record.Alt,
      });
    });
    ...
    if (airData[current]) {
      ...
      airData[current].forEach((data) => {
        ...
        jsonData.data.push({
          ...
          rotation: [data.Heading, data.Pitch, data.Roll], // XYZ 三轴旋转度数 [偏航角（单位：角度，0正北, -360~360), 俯仰角（单位：角度，0水平, -360~360),翻滚角（单位：角度，0水平, -360~360)]
...
function followingCameraBtn(id, category) {
  ...
  let jsonData = {
    ...
    relative: 'self', // 视角的相对位置，"self"，视角相对模型固定，"world"，视角相对世界固定。鼠标可以支持环绕模型旋转和缩放。
  };
...
function setBaseCenter() {
  let jsonData = {
    originLon: 40.59508025821, // 中心点经度
    originLat: 44.84466015014, // 中心点纬度
    originHeight: 0, // 单位m
  };
...
function aircraftTabClick(int) {
  ...
  if (int === 1) {
    ...
    console.log(currentTime, JSON.stringify(blueArray));
...
function getJsonData() {
  var p1 = new Promise((resolve, reject) => {
    ...
    xhr.open('GET', './json/blacksea.json', true);
  ...
  var p2 = new Promise((resolve, reject) => {
    var xhr = new XMLHttpRequest();
    xhr.open('GET', './json/aircraftgps.json', true);
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4 && xhr.status === 200) {
        var res = JSON.parse(xhr.responseText);
        resolve(res.RECORDS);
  ...
  var p3 = new Promise((resolve, reject) => {
    var xhr = new XMLHttpRequest();
    xhr.open('GET', './json/missilecraftgps.json', true);
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4 && xhr.status === 200) {
        var res = JSON.parse(xhr.responseText);
        resolve(res.RECORDS);
  ...
  var p4 = new Promise((resolve, reject) => {
    var xhr = new XMLHttpRequest();
    xhr.open('GET', './json/destroyplane.json', true);
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4 && xhr.status === 200) {
        var res = JSON.parse(xhr.responseText);
        resolve(res.RECORDS);
  ...
  Promise.all([p1, p2, p3, p4]).then((res) => {
    addLandmarkLayer(res[0]);
    resolveTrailData(res[1], 'air');
    resolveTrailData(res[2], 'missile');
    resolveDestroyData(res[3]);
    addModelTrailLayer('air');
    addModelTrailLayer('missile');
    startTime = convertDate(airStartTime);
    endTime = convertDate(airEndTime);
...
// 处理轨迹图数据
function resolveTrailData(records, category) {
  this[category + 'StartTime'] = records[0].Time;
  this[category + 'EndTime'] = records[records.length - 1].Time;
  let index = 1;
  for (let i = 0; i < records.length; i++) {
    if (!this[category + 'Data'][records[i].Time]) {
      this[category + 'Data'][records[i].Time] = [];
      this[category + 'DataLength'] += 1;
    }
    if (category != 'destroy' && !this[category + 'Legend'][records[i].Id]) {
      this[category + 'Legend'][records[i].Id] = {
        name: 'legendName' + index,
      };
      index++;
    }
    this[category + 'Data'][records[i].Time].push(records[i]);
  }
  timeParameter();
}

// 处理销毁数据
function resolveDestroyData(records) {
  for (let i = 0; i < records.length; i++) {
    if (!destroyData[records[i].datetime]) {
      destroyData[records[i].datetime] = [];
      destroyDataDataLength += 1;
    }
    destroyData[records[i].datetime].push(records[i]);
  }
  timeParameter();
}
...
function convertDate(timeStr) {
  let dt = timeStr.split(' ');
  return new Date(dt[0].split('-').reverse().join('-') + ' ' + dt[1]);
}

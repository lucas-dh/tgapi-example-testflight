...
var sortAnimationList = []; // 根据时间轴关键帧动画的第一次出现的先后顺序排序的动画播放列表 关节数据没有顺序排列用于设置对应时刻的关节状态
// sortAnimationList = ['一级分离', '整流罩分离', '调姿', '二级分离', '三级分离']

// 动画数据和html关键帧按钮索引对应
var timelineAnimationList = [
  {
    animationName: '一级分离', // 动画名称
  ...
  },
  {
    animationName: '三级分离',
    useAnimation: true,
    start: new Date('2009-07-14 18:28:00').getTime(),
    state: 0,
    runAnimationRocketList: [
      {
        name: '火箭2',
  ...
  },
  {
    animationName: '一级分离',
    useAnimation: true,
    start: new Date('2009-07-14 18:30:00').getTime(),
    state: 0,
    runAnimationRocketList: [
      {
        name: '火箭1',
  ...
  //   animationName: '二级分离',
  //   useAnimation: true,
  //   start: new Date('2009-07-14 18:32:00').getTime(),
  //   state: 0,
  // },
  // {
  //   animationName: '三级分离',
  //   useAnimation: true,
  //   start: new Date('2009-07-14 18:34:00').getTime(),
  //   state: 0,
  // },
];
...
function connctServer() {
  let jsonData = {
    name: 'TwinEasyServer',
    address: 'http://172.16.5.30:5000/v1/public/location/pnu1vtr7zoowbpy8/twinEasyCategoryId/{iconModelId}',
    token: 'ABCDEFG',
  };
  ...
  if (startData) {
    ...
    rocketInfoList.forEach((rocket) => {
      jsonData.data.push({
        ...
        coordZ: (data.Alt + rocket.offset) * 1000.0 + 130,
 ...
 *
 * 设置关节两种类型 设置当前动画需要执行的模型关节 和 单独播放动画时另外不需要执行当前动画的关节
 *
 * 根据当前点击时间轴的动画数据获取需要执行的播放动画的模型列表 `currentClickAnimationData.runAnimationRocketList` 分两步执行
 *
 * 1.找到当前点击动画需要执行动画的模型里的关节，设置关节状态根据当前点击的动画名称占时间轴动画排序数组的索引值和关节所属的动画名称占排序数组的索引值比较，设置关节的状态。
 * 如果当前点击动画名称所占的索引值大于关节所属动画名称所占的索引值，则将关节状态设置为完成状态，否则将关节状态设置为初始状态。
 * 2.找到当前点击动画不需要执行动画的其他模型的关节，设置其他关节时方法一致，设置当前动画下一致的关节时 如果另外模型的同名动画在当前点击的动画前面 另外的模型动画关节置为完成状态
 *
 * 设置好关节数据然后通过 `uniCall` 方法调用 `setModelArticulation` 接口来更新模型的关节状态。
 *
 */
function setArticulation(currentClickAnimationData) {
  // 获取当前点击的动画名称
  let currentClickAnimationName = currentClickAnimationData.animationName;

  // 1.设置当前点击动画需要执行动画的模型里的关节
  currentClickAnimationData.runAnimationRocketList.forEach((rocket) => {
    // 遍历的当前动画名称的占排序列表的索引值 小于 当前点击的动画名称占排序列表的索引值时 关节置为完成状态， 否则关节置为初始状态
    // 遍历关节数据 根据点击的动画名称和当前的关节数据动画名称占排序后的动画列表的索引值进行比较设置

    // 当前模型关节的设置后的数据
    let currentArticulationValue = rocketInfoList
      // 找到当前点击的需要播放动画的模型里的所有关节项
      .find((item) => item.name == rocket.name)
      .articulationItems.map((item) => ({
        articulation: item.articulationName,
        type: item.type,
        value: sortAnimationList.indexOf(item.animationName) < sortAnimationList.indexOf(rocket.animationName) ? item.endState : item.startState,
      }));
  ...
  // 2.找到这次点击不需要播放动画的其他模型里的关节 进行设置相应的状态
  rocketInfoList
    .filter((item) => currentClickAnimationData.runAnimationRocketList.findIndex((t) => t.name == item.name) == -1)
    // 通过名称过滤出这次不需要播放动画的模型信息列表
    .forEach((rocket) => {
      // 获取到当前模型的关节数据 并设置相关状态
      let currentArticulationValue = rocket.articulationItems.map((item) => {
        let articulationData = {
          articulation: item.articulationName,
          type: item.type,
          value: sortAnimationList.indexOf(item.animationName) < sortAnimationList.indexOf(currentClickAnimationName) ? item.endState : item.startState,
        };

        // 如果当前点击的模型动画对应的是当前关节 并且在时间轴列表中找到相同动画的数据 根据开始时间找到是否有小于当前点击时间的相同动画，如果有这个动画则对应的关节置为完成状态
        // （解决单独执行一个模型的动画时 其他模型的相同动画下和当前点击的模型在不同时间先后顺序的关节状态）
        if (
          item.animationName == currentClickAnimationName &&
          timelineAnimationList
            .filter((item) => item.runAnimationRocketList.findIndex((t) => t.animationName == currentClickAnimationName) > -1)
            .findIndex((item) => item.start < currentClickAnimationData.start) > -1
        ) {
          articulationData.value = item.endState;
        }
...
function getModelAnimationAndSetArticulation() {
  ...
  // 设置动画播放排序后的数组
  timelineAnimationList.forEach((item) => {
    if (!sortAnimationList.includes(item.animationName)) {
      sortAnimationList.push(item.animationName);
    }
  });

  // console.log('排序后的动画播放列表', sortAnimationList);
}

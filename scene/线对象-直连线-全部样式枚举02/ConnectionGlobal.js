  ...
  var vm = new Vue({
    ...
    methods: {
      ...
      addCapital(inJsonArr) {
        let _this = this;
        let counter = inJsonArr.capital.length;
        inJsonArr.capital.forEach(function (item, index) {
          let obj = {
            ID: item.ID,
            name: item.name,
            lon: item.lon,
      ...
      addConnection(inJsonArr) {
        let _this = this;
        let counter = parseInt(inJsonArr.capital.length) - 1;
        inJsonArr.capital.forEach(function (item, index) {
          if (item.name != "北京") {
            let typeIdx =
            ...
            appInstance.uniCall("addConnection", jsonData, (result) => {
              console.log("到" + item.name + "的直连线添加结束");
              console.log(result);
